...
/**
 * Upload one voice recording and insert a message to each of the given groups (challenge reply broadcast).
 * @param {{ uri: string, duration: number }} audioResult - duration in ms
 * @param {{ group_id: string, challenge_id: string }[]} groupsWithChallengeId - list of { group_id, challenge_id }
 * @param {string} userId
 * @returns {Promise<void>}
 */
export async function uploadChallengeVoiceToGroups(audioResult, groupsWithChallengeId, userId) {
    if (!audioResult?.uri || !groupsWithChallengeId?.length || !userId) throw new Error('Missing args for uploadChallengeVoiceToGroups');
    const { uri, duration } = audioResult;
    const audioData = await FileSystem.readAsStringAsync(uri, { encoding: FileSystem.EncodingType.Base64 });
    const fileName = `language-chat/${userId}/voice_challenge_broadcast_${Date.now()}.m4a`;
    const { error: uploadError } = await supabase.storage
        .from('voice-memos')
        .upload(fileName, decode(audioData), { contentType: 'audio/m4a' });
    if (uploadError) throw uploadError;
    ...
    const inserts = groupsWithChallengeId.map(({ group_id, challenge_id }) =>
        supabase.from('app_messages').insert({
            sender_id: userId,
            group_id,
    ...
    for (const { group_id } of groupsWithChallengeId) {
        supabase.functions.invoke('notify-challenge-reply', {
            body: { group_id, sender_id: userId },
        }).catch(() => {});
    }
    ...
    await supabase
        ...
        .update({ last_read_at: new Date().toISOString() })
        .eq('user_id', userId)
        .in('group_id', groupsWithChallengeId.map(g => g.group_id));
}
