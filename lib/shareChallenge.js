...
export async function shareChallenge(userId, groupId, messageId, language) {
    try {
        ...
        const shareMessage = `bet ur still too scared to speak ${language} no presh but i already did the challenge 🤪 ${shareUrl} (expires in 24 hours)`;
        ...
        if (isAvailable) {
            ...
            Alert.alert('Share link created', shareMessage);
            return true;
        }
    } catch (error) {
