...
serve(async (req) => {
    ...
    try {
        ...
        formData.append('response_format', 'verbose_json') // Get word-level confidence
        ...
        let avgConfidence = 0.95 // Default high confidence
        if (whisperResult.words && whisperResult.words.length > 0) {
            const totalConfidence = whisperResult.words.reduce((sum, word) => sum + (word.confidence || 0), 0)
...
Keep explanations SHORT and helpful. If grammar is perfect, set is_correct to true and leave explanation empty.`
        ...
        const groqResponse = await fetch('https://api.groq.com/openai/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                temperature: 0.3, // Original working temperature
            }),
        })
        ...
        // Use 'Rachel' (Neutral US English) as default voice
        ...
        const elevenLabsResponse = await fetch(
            `https://api.elevenlabs.io/v1/text-to-speech/${voiceId}`,
            {
                ...
                body: JSON.stringify({
                    text: correction.corrected, // Use CORRECTED text
        ...
        const pronunciationUrl = urlData.publicUrl
        ...
        return new Response(
            JSON.stringify({
                ...
                pronunciationUrl,
                confidence: avgConfidence, // 0.0 to 1.0
            }),
