// Set emoji password for an existing account that has none (early user).
// Called when claim_user_identity returns ACCOUNT_EXISTS_NO_PASSWORD.
...
serve(async (req) => {
    ...
    try {
        ...
        if (!rows?.length || rows.length !== 1) {
            return new Response(JSON.stringify({ error: rows?.length === 0 ? 'no matching early user' : 'multiple matches' }), {
                status: 400,
                headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
            });
        }
        ...
        console.error('set-early-user-emoji-password error:', e);
