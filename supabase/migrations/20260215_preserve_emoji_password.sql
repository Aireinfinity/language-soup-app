-- Never overwrite existing emoji_password. Users rely on it to log in; if we overwrite or clear it, they lose access.
-- This migration updates claim_user_identity so ON CONFLICT only sets emoji_password when the existing row has none.
...
BEGIN
  ...
  ON CONFLICT (id) DO UPDATE SET
    ...
    -- NEVER overwrite existing emoji_password: user needs it to log in
...
COMMENT ON FUNCTION claim_user_identity(TEXT, TEXT) IS 'Claim or create app_users row. Never overwrites existing emoji_password.';
